    ...
    try {
        ...
        if (response.ok) {
            ...
            hotels.forEach(function(h) {
                console.log("🏨", h.name, "Slug:", h.slug, "Rooms:", h.rooms ? h.rooms.length : 0);
            });
    ...
    grid.innerHTML = list.map(function(h, i) {
        ...
        return '<div class="card" style="--d:' + (i * 50) + 'ms">' +
            ...
            '<div class="card-actions">' +
                ...
                '<button onclick="openScannerForHotel(\'' + h.id + '\')">📱 Scan</button>' +
            '</div></div>';
    }).join("");
}

// Open Scanner for Specific Hotel
function openScannerForHotel(hotelId) {
    console.log("🔍 openScannerForHotel called with ID:", hotelId);
    currentHotelId = hotelId;
    var hotel = hotels.find(function(h) { return h.id === hotelId; });
    if (hotel) {
        console.log("🏨 Current hotel set to:", hotel.name, "Slug:", hotel.slug);
...
function enterRoomFromScan() {
    ...
    if (currentHotelId) {
        ...
        if (currentHotel) {
            ...
            if (currentHotel.rooms && currentHotel.rooms.length) {
                roomFound = currentHotel.rooms.find(function(r) {
                ...
                if (roomFound) {
                    ...
                    alert("❌ Room " + rn + " not found in " + currentHotel.name + ". Available rooms: " + 
                          currentHotel.rooms.map(function(r) { return r.room_number; }).join(", "));
                    return;
                }
            } else {
    ...
    // If no hotel was selected, search all hotels
    ...
    console.log("🚀 Navigating to: /guest/room/?hotel=" + slug + "&room=" + rn);
    window.location.href = "/guest/room/?hotel=" + slug + "&room=" + rn;
}
...
function openScanner() {
    ...
    if (hotelNameSpan) {
        if (currentHotelId) {
            ...
            hotelNameSpan.textContent = hotel ? hotel.name + " (Slug: " + hotel.slug + ")" : "None selected";
        } else {
